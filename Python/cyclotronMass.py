...
def plot_differences_with_points(Datenreihen, BTable, sigmaXXTable, farben, selected_points):
    ...
    plt.ylabel("$\sigma_{XX} / \sigma_{fit} -1$ ", fontsize = 20)
...
def calculate_cyclotron_mass(BPeak, BPeakFehler, Amplitudes, AmplitudesFehler):
    q = np.array([Amplitudes[0] / Amplitudes[2], Amplitudes[1] / Amplitudes[3]])
    ...
    cyclotronElectron15 = (const.hbar * const.e * BPeak) / (const.m_e * np.pi**2 * const.k * 1.5) * np.arccosh(q[0])
    cyclotronElectron21 = (const.hbar * const.e * BPeak) / (const.m_e * np.pi**2 * const.k * 2.1) * np.arccosh(q[1])
    a15 = (const.hbar * const.e) / (const.m_e * np.pi**2 * const.k * 1.5)
    a21 = (const.hbar * const.e) / (const.m_e * np.pi**2 * const.k * 2.1)
    cyclotron15Error = np.sqrt((a15*np.arccosh(q[0])*BPeakFehler)**2+(a15*BPeak*(1/(np.sqrt(q[0]**2+1))*qError[0]))**2)*const.m_e
    cyclotron21Error = np.sqrt((a21*np.arccosh(q[1])*BPeakFehler)**2+(a21*BPeak*(1/(np.sqrt(q[1]**2+1))*qError[1]))**2)*const.m_e
...
def plot_data_and_fits_with_points(Datenreihen, BTable, sigmaXXTable, farben):
    ...
    plt.ylabel(r"$\sigma_{\text{xx}} / S$", fontsize = 20)
    plt.xlim(1,1.5)
    plt.ylim(1,3.5)
    plt.legend(fontsize = 12)
    plt.tick_params(axis='both', which='major', labelsize=16, direction = 'in')
